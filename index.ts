...
const main = async () => {
  ...
  const eventData = await page.evaluate(() => {
    const eventArticles = Array.from(
      document.querySelectorAll(".os-c-list-cmn")
    );
    return eventArticles.map((evt) => {
      const imageUrl = evt.querySelector("img")?.getAttribute("src");
      const title = evt.querySelector(
        ".os-c-list-cmn__title-link"
      )?.textContent;
      const location = evt.querySelector(".os-c-list-cmn__disc")?.textContent;
      const details = evt.querySelector(
        ".os-c-list-cmn__lead.os-c-list-cmn-tile-event-lead"
      )?.textContent;
